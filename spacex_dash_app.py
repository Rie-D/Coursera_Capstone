...
def pie(site_dropdown):
    if site_dropdown == 'All':
        ...
        pie_fig=px.pie(spacex_df,values='class', names='Launch Site', title=title_pie)
        return pie_fig
    else:
        filtered_pie=spacex_df[spacex_df['Launch Site'] == site_dropdown]
        class_pie=filtered_pie.groupby(['Launch Site', 'class']).reset_index(name='class count')
        title_pie=f'Success Launches for site {site_dropdown}'
        pie_fig=px.pie(class_pie, values='class count', names='class', title=title_pie)
        return pie_fig       
...
def scatter(site_dropdown, slider_range):
    ...
    if site_dropdown == 'All':
        title_scatter=f'Success Launches for site {site_dropdown}'
        scatter_fig=px.scatter(df_scatter, x='Payload Mass (kg)', y='class', color='Boosster Version Category',title=title_scatter)
        return scatter_fig
    else:
        ...
        scatter_fig=px.scatter(filtered_scatter, x='Payload Mass (kg)', y='class', color='Boosster Version Category', title=title_scatter)
        return scatter_fig
